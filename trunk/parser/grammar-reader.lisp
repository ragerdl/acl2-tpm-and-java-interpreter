...
(define read-next-bnf-lexeme1 ((file-input character-listp)
  ...
  :prepwork 
  ((defthm firstn-chars-lemma 
     (implies (and (stringp lexeme) 
                   (< 0 (length lexeme))) 
              (consp (str::firstn-chars 1 lexeme))) 
     :hints (("Goal" :in-theory (enable str::firstn-chars)))))
...
(local
 (include-book "std/ks/intern-in-package-of-symbol" :dir :system))
...
; read-line
      ...
      :hints (("Goal" :in-theory (enable read-lexicon-line))
              ("Goal'''" :in-theory (e/d (read-lexicon-line) (read-line!-strong))
